...
<head>
    ...
    <title>Add Client - Codeban CMS</title>
...
<body class="ve-cms-body">
    ...
    <main class="ve-cms-main">
        <header class="ve-cms-topbar"><div><span class="ve-section-tag">Clients</span><h1>Add Client</h1></div><div class="ve-cms-actions"><a href="{{ route('cms.clients.index') }}" class="ve-cms-view-site"><i class="fa fa-arrow-left"></i> Back</a></div></header>
        @if ($errors->any())<div class="ve-cms-alert">{{ $errors->first() }}</div>@endif
        <section class="ve-cms-form-panel"><form action="{{ route('cms.clients.store') }}" method="post" enctype="multipart/form-data">@csrf @include('cms.clients.form')</form></section>
    </main>
</body>
</html>
