<div class="ve-cms-form-grid">
    ...
    <div class="ve-cms-field"><label for="category">Category</label><input id="category" name="category" value="{{ old('category', $galleryImage->category) }}" type="text" placeholder="PPE, Uniforms, Branding"></div>
    ...
    <div class="ve-cms-field ve-cms-field-wide"><label for="image">Gallery Image</label><input id="image" name="image" type="file" accept="image/*" @required(! $galleryImage->exists)>@if ($galleryImage->image)<div class="ve-cms-image-preview" style="background-image:url({{ asset($galleryImage->image) }});"></div>@endif</div>
</div>
