...
<head>
    ...
    <title>Add Testimonial - Codeban CMS</title>
...
<body class="ve-cms-body">
    ...
    <main class="ve-cms-main">
        <header class="ve-cms-topbar"><div><span class="ve-section-tag">Testimonials</span><h1>Add Testimonial</h1></div><div class="ve-cms-actions"><a href="{{ route('cms.testimonials.index') }}" class="ve-cms-view-site"><i class="fa fa-arrow-left"></i> Back</a></div></header>
        @if ($errors->any())<div class="ve-cms-alert">{{ $errors->first() }}</div>@endif
        <section class="ve-cms-form-panel"><form action="{{ route('cms.testimonials.store') }}" method="post" enctype="multipart/form-data">@csrf @include('cms.testimonials.form')</form></section>
    </main>
</body>
</html>
